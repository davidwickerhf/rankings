"""
Test composite measures using LOW-RELEVANCE PRIORITY approach.

This script implements the alternative approach mentioned in the paper (lines 553-554):
"Judgments ranked in ascending order based on the measure that performed well at 
identifying low-relevance judgments (with rank 1 being the least relevant). The bottom 
n judgments from this list were selected, after which the remaining judgments were 
ranked using the measure effective at identifying highly relevant judgments."

Key difference from script 04 (High-Relevance Priority):
- Script 04: Prioritizes top n% from HIGH centrality, then ranks rest by LOW centrality
- Script 06: Prioritizes bottom n% from LOW centrality, then ranks rest by HIGH centrality

Uses the same optimal thresholds found in script 04 for direct comparison.
...
OUTPUT_DIR = 'results/analysis/06_low_relevance_priority'
...
# ==================== COMPOSITE FUNCTIONS ====================
def create_low_relevance_priority_composite(df, high_centrality, low_centrality, ground_truth, threshold):
    """
    Create composite ranking using LOW-RELEVANCE PRIORITY approach.
    
    Methodology:
    1. Rank all by LOW centrality (descending)
    2. Select BOTTOM n% (those with LOWEST low-centrality) - these are the least relevant
    3. Assign them ranks N-k+1 to N
    4. Remaining judgments: rank by HIGH centrality, assign ranks 1 to N-k
    """
    n_total = len(df)
    n_threshold = int(threshold * n_total)
    
    # Rank by low-relevance centrality (higher = better at low-relevance detection)
    low_ranks = df[low_centrality].rank(method='first', ascending=False)
    
    # Rank by high-relevance centrality (higher = better at high-relevance detection)
    ...
    # Select BOTTOM n_threshold from low-relevance ranking (worst low-centrality scores)
    # These are the indices with the LARGEST ranks in low_ranks (i.e., lowest actual centrality values)
    bottom_low = low_ranks.nlargest(n_threshold).index
    
    # Remaining judgments (those NOT in bottom)
    remaining = df.index.difference(bottom_low)
    
    # Assign ranks:
    # 1. Remaining judgments: ranked by HIGH centrality (ranks 1 to N-k)
    if len(remaining) > 0:
        remaining_high_ranks = high_ranks.loc[remaining].rank(method='first', ascending=True)
        composite_ranks.loc[remaining] = remaining_high_ranks
    
    # 2. Bottom judgments: shift to end (ranks N-k+1 to N)
    if len(bottom_low) > 0:
        bottom_low_ranks = low_ranks.loc[bottom_low].rank(method='first', ascending=False)
        composite_ranks.loc[bottom_low] = (n_total - n_threshold) + bottom_low_ranks
...
def analyze_network_with_threshold(df, high_cent, low_cent, ground_truth, threshold):
    ...
    composite_ranks, composite_corr = create_low_relevance_priority_composite(
...
def test_with_optimal_threshold(all_networks, high_cent, low_cent, combo_name, optimal_thresholds):
    """Test combination using optimal thresholds found via grid search."""
    print(f"\n{'='*80}")
...
def main():
    ...
    for high_cent, low_cent, combo_name in COMBINATIONS_TO_TEST:
        ...
        # Optimize thresholds for both ground truths
        optimal_thresholds = {}
        threshold_data = {}
        
        for ground_truth in GROUND_TRUTHS:
            optimal_threshold, threshold_wins = optimize_threshold(
                all_networks, high_cent, low_cent, ground_truth
            )
            optimal_thresholds[ground_truth] = optimal_threshold
            threshold_data[ground_truth] = dict(threshold_wins)
        
        # Test with optimal thresholds
        results, detailed_results = test_with_optimal_threshold(
            all_networks, high_cent, low_cent, combo_name, 
            optimal_thresholds
        )
    ...
    print("\n" + "="*80)
    print("ANALYSIS COMPLETE - LOW-RELEVANCE PRIORITY APPROACH")
